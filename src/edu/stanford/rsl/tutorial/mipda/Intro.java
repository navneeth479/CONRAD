...
public class Intro {
	...
	public void basicIntro() {
		...
		System.out.println("Col Vector is " + colVector.toString());
		...
		v2 = null;//TODO
		v2 = colVector.dividedBy(vecNormL2);
		...
		//TODO 
		tempColVector = tempColVector.dividedBy(vecNormL2);
		if (v2 != null)
	...
	public void gridIntro(){
		...
		int height =image.getHeight();
			...
			for(int x = -radius; x<=radius; x++)
			{
				...
				{//Check for point location in or outside the circle
					if(x*x + y*y <= (radius*radius))
					{
						double[] idx = image.physicalToIndex(x, y);
						int i = (int) idx[0];
						int j = (int) idx[1];
						
						if(i>=0 && j>=0 && i<width && j<height)
						{
							image.setAtIndex(i, j, (float) insideVal );
						}
					}
				}
			}
		...
		mrImage = null;//TODO
       	String filename = "C:\\Users\\navne\\Desktop\\WiSe_21-22\\MIDP\\Exercises\\CONRAD\\data\\mipda\\T1.png";
       	ImagePlus img = IJ.openImage(filename);
       	mrImage = ImageUtil.wrapImagePlus(img).getSubGrid(0);
		...
		conv.convolveFloat(ip, kernel, kw, kh);
		if (ip != null) {
		...
		ImagePlus imgp = new ImagePlus("Image",ImageUtil.wrapGrid2D(convolvedImage).createImage());
		outFileName = "C:\\Users\\navne\\Desktop\\WiSe_21-22\\MIDP\\Exercises\\CONRAD\\data\\mipda\\output\\" + imageFileName;
		if (mrImage != null) {
